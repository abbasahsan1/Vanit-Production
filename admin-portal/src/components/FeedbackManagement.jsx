...
const FeedbackManagement = () => {
  ...
  const fetchFeedbacks = async () => {
    try {
      ...
      console.log('📡 Making API calls to fetch feedback...');
      ...
      const [studentResponse, captainResponse] = await Promise.all([
        ...
        axios.get('http://localhost:5000/api/feedback/captain/all')
      ]);
      
      console.log('✅ Student feedback response:', studentResponse.data);
      console.log('✅ Captain feedback response:', captainResponse.data);

      // Combine and format feedbacks
      const studentFeedbacks = (studentResponse.data || []).map(feedback => ({
        ...feedback,
        type: 'student',
      ...
      const captainFeedbacks = (captainResponse.data || []).map(feedback => ({
        ...feedback,
        type: 'captain',
      ...
      console.log(`📊 Processed ${studentFeedbacks.length} student feedbacks`);
      console.log(`📊 Processed ${captainFeedbacks.length} captain feedbacks`);

      const allFeedbacks = [...studentFeedbacks, ...captainFeedbacks].sort(
        (a, b) => new Date(b.created_at) - new Date(a.created_at)
      );

      console.log(`📋 Total feedbacks combined: ${allFeedbacks.length}`);
  ...
  const handleStatusUpdate = async (feedbackId, newStatus) => {
    try {
      ...
      const endpoint = feedback.type === 'student' 
        ...
        : `http://localhost:5000/api/feedback/captain/${feedbackId}/status`;
